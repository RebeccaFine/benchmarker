...
parser.add_argument('--label1', help = 'Label for first tested annotation (L2_2) (should not include trait name). In the suggested pipline, this is the same label as used throughout the LD score and h2 calculation')
parser.add_argument('--label2', help = 'Label for second tested annotation (L2_3) (should not include trait name). In the suggested pipline, this is the same label as used throughout the LD score and h2 calculation')
...
original_results = original_results[ original_results.Category.isin(['L2_2','L2_3'])].copy()
print 'Assuming L2_2, L2_3 are the prioritized annotations'
original_columns = original_results.columns.tolist()
original_results['Trait'] = trait
original_results[ 'Label'] = original_results.Category.map( {'L2_2':label1, 'L2_3':label2} )
